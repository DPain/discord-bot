...
public class AnimeTorrentFinder {
  private final static Logger logger = LoggerFactory.getLogger(AnimeTorrentFinder.class);
  ...
  public AnimeTorrentFinder() {
    try {
      entryLimit =
          Integer.parseInt(PropertiesManager.load().getValue(Property.TORRENT_ENTRY_LIMIT));
    } catch (NumberFormatException e) {
      logger.error(String.format("Property: %s is not formatted correctly!",
  ...
  public LinkedList<String> searchTorrent(String name) throws IOException {
    ...
    Connection connection = Jsoup.connect(parseLink).userAgent(
