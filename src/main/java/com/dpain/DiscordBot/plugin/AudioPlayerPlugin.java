	...
	public void handleEvent(Event event) {
		if (event instanceof GuildMessageReceivedEvent) {
			try {
				...
				if (canAccessPlugin(castedEvent.getMember())
						&& !castedEvent.getAuthor().getId().equals(castedEvent.getJDA().getSelfUser().getId())) {
					if (message.startsWith("-")) {
						...
						if (message.startsWith("-join ")) {
							...
							try {
								...
								String temp1 = LogHelper.elog(castedEvent,
										String.format("Incorrect command: %s", message));
